...
const OCCUPATION_LABEL = {
  pelajar: "Pelajar (SMP/SMA/SMK/MA)",
  mahasiswa: "Mahasiswa",
  guru: "Guru / Pengajar",
  non_akademisi: "Non-Akademisi",
  lainnya: "Lainnya",
};

const UserProfile = ({ setActiveTab }) => {
  const { profile } = useAuthStore();
  const { latestResult } = useResultStore();
  ...
  return (
    <div className="space-y-6 animate-fade-in w-full">
      ...
      <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center text-center space-y-4">
        ...
        <span className="bg-teal-50 text-[#008080] text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider">
          {OCCUPATION_LABEL[profile?.occupation] ||
            profile?.occupation ||
            "Umum"}
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-6">
          ...
          <div className="pt-4 space-y-3">
            ...
            <div className="bg-edu-bg border border-gray-100 p-4 rounded-xl flex items-center gap-3">
              ...
              {pace.desc && (
                <span className="text-xs text-edu-text-gray">
                  — {pace.desc}
                </span>
              )}
            </div>
